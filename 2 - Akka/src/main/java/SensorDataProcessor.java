...
public class SensorDataProcessor {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < NO_SENSORS; i++) {
			sensors.get(i).tell(new ConfigMsg(dispatcher), ActorRef.noSender());
		}
		...
		//Test: adding actor during runtime
		/*ActorRef runtimeSensor = sys.actorOf(TemperatureSensorActor.props(), "runtime");
		sensors.add(runtimeSensor);
		runtimeSensor.tell(new ConfigMsg(dispatcher), ActorRef.noSender());*/
		...
		for (int i = 0; i < 3; i++) {
			sensors.get(0).tell(new GenerateMsg(), ActorRef.noSender());
			try{
				Thread.sleep(1500);
			}catch (Exception e){
